  ...
  return (
    <div className="space-y-5 pb-28 max-sm:space-y-4">
      <div className="relative overflow-hidden rounded-3xl border border-[var(--border)] bg-[var(--bg-elevated)] p-6 shadow-lg shadow-[var(--shadow)] backdrop-blur max-sm:rounded-2xl max-sm:p-5">
        <div className="pointer-events-none absolute -right-20 -top-20 h-60 w-60 rounded-full bg-[#E53935]/10 blur-[100px] max-sm:hidden" />
        <div className="relative">
          <Button variant="ghost" className="mb-3" onClick={() => router.back()}>← Back</Button>
          <div className="flex items-center gap-3">
            <span className={`h-3 w-3 rounded-full ${
              bus.status === 'running' ? 'bg-[#E53935] shadow-[0_0_12px_rgba(229,57,53,0.6)]' :
              bus.status === 'delayed' ? 'bg-[#FFB300] shadow-[0_0_12px_rgba(255,179,0,0.5)]' :
              'bg-[var(--text-muted)]'
            }`} />
            <p className="font-orbitron text-[10px] uppercase tracking-[0.3em] text-[#E53935]">{t('app.name')}</p>
          </div>
          <h1 className="mt-2 text-2xl font-semibold text-[var(--text-primary)] max-sm:text-xl">Bus {bus.number}</h1>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4">
        <div className="rounded-3xl border border-[var(--border)] bg-[var(--bg-elevated)] p-4 shadow-lg shadow-[var(--shadow)] backdrop-blur max-sm:rounded-2xl max-sm:p-3">
      ...
      <div className="grid gap-4 md:grid-cols-2 md:gap-6">
        <div className="rounded-3xl border border-[var(--border)] bg-[var(--bg-elevated)] p-5 shadow-lg shadow-[var(--shadow)] backdrop-blur max-sm:rounded-2xl max-sm:p-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-2xl border border-[#E53935]/30 bg-[#E53935]/8 p-3">
              <p className="text-[10px] uppercase tracking-[0.25em] text-[#E53935]">{t('bus.currentStop')}</p>
            ...
            return (
              <li
                ...
                className={`rounded-2xl p-3 transition ${
                  idx === currentIndex
                    ? 'border border-[#E53935]/30 bg-[#E53935]/10 text-[var(--text-primary)]'
                    : idx === currentIndex + 1
                      ? 'border border-[#00BCD4]/30 bg-[#00BCD4]/10 text-[var(--text-primary)]'
                      : idx < currentIndex
                        ? 'bg-black/[0.02] text-[var(--text-muted)]'
                        : 'bg-black/[0.03] text-[var(--text-secondary)]'
                }`}
              >
                ...
                <div className="flex flex-1 items-center justify-between">
                  <span>
                    {stop.name}
                    {idx === currentIndex && <span className="ml-2 inline-block h-2 w-2 rounded-full bg-[#E53935] shadow-[0_0_8px_rgba(229,57,53,0.6)]" />}
