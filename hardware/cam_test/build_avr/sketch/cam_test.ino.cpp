#include <Arduino.h>
#line 1 "C:\\Users\\NIKILAN\\transit-dashboard\\hardware\\cam_test\\cam_test.ino"
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED && millis() - lastSend > 15000) {
    lastSend = millis(); sendCount();
  }
...
#line 1 "C:\\Users\\NIKILAN\\transit-dashboard\\hardware\\cam_test\\cam_test_simple.ino"
#include <esp_camera.h>
#include <WiFi.h>
#include "esp_http_server.h"
...
void startServer() {
  ...
  if (httpd_start(&httpd, &config) == ESP_OK)
    for (int i = 0; i < 2; i++) httpd_register_uri_handler(httpd, &r[i]);
}

void setup() {
  pinMode(LED_PIN, OUTPUT); digitalWrite(LED_PIN, LOW); delay(200);
...
#line 1 "C:\\Users\\NIKILAN\\transit-dashboard\\hardware\\cam_test\\minimal.ino"
